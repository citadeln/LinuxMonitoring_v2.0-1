...
cleanup_by_log() {
    ...
    # -F'[ :(]' говорит awk: разделяй строку по пробелам, двоеточиям и открывающей скобке.
    # $NF теперь будет содержать полный путь к папке без даты в скобках.
    ...
    done < <(awk -F'[ :(]' '/^Created folder: / {print $NF}' "$base_dir/$log_file")
    ...
    done < <(awk -F'[ :(]' '/^Created file: / {print $NF}' "$base_dir/$log_file")
