...
# HTTP Methods:
# GET: Запросить данные с сервера (чтение). Самый частый метод (просмотр страниц).
# POST: Отправить данные на сервер для создания нового ресурса (например, отправить форму регистрации).
# PUT: Обновить существующий ресурс на сервере (полная замена данных).
# PATCH: Частично обновить существующий ресурс (изменить только часть данных).
